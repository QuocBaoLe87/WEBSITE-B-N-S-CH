...
import com.v.model.ChatMessage;
import com.v.model.ChatMessageEntity;
import com.v.repository.ChatMessageRepository;
...
public class ChatRestController {
    ...
    public List<PartnerDto> partners(Principal principal) {
        ...
        try {
            ...
            log.error("Failed to load latest threads for '{}'", meCanonical, ex);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                    "Failed to load chat partners: " + ex.getMessage(), ex);
        }
        ...
        if (!meReal.equalsIgnoreCase(meCanonical)) {
            try {
                ...
                log.error("Failed to load legacy threads for '{}'", meReal, ex);
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                        "Failed to load chat partners (legacy): " + ex.getMessage(), ex);
            }
        }
    ...
    // DEBUG endpoint - kiểm tra raw data từ DB
    @GetMapping("/debug/partners")
    @PreAuthorize("hasRole('ADMIN')")
    public Object debugPartners(Principal principal) {
        String me = canonicalMe(principal);
        try {
            return repo.findLatestThreads(me);
        } catch (Exception e) {
            log.error("Debug endpoint failed", e);
            return Map.of("error", e.getMessage(), "trace", Arrays.toString(e.getStackTrace()));
        }
    }
}
