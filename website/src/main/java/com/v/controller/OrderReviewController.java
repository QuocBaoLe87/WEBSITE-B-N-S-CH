...
public class OrderReviewController {
    ...
    public ResponseEntity<?> create(@AuthenticationPrincipal UserDetails ud,
        ...
        boolean exists = order.getItems().stream()
                .anyMatch(i -> i.getProduct() != null && i.getProduct().getId().equals(bookId));
        if (!exists) {
        ...
        // Nếu entity Review của dự án có setOrder(Order) thì liên kết, nếu không có
        // cũng không sao.
        try {
            r.getClass().getMethod("setOrder", Order.class).invoke(r, order);
