...
import com.v.dto.TopProductDto;
import com.v.exception.EmailExistsException;
import com.v.exception.UsernameExistsException;
import com.v.model.*;
import com.v.repository.CategoryRepository;
...
public class AdminController {
    ...
    private final com.v.service.BrandService brandService;
    ...
    public AdminController(ProductService productService,
            ...
            com.v.service.BrandService brandService) {
    ...
    public String dashboard(
        ...
        List<String> revenueStatusNames = revenueStatuses.stream().map(Enum::name).toList();
        List<Object[]> rows = orderRepository.revenueDailyBetween(start7, endToday, revenueStatusNames);
    ...
    public String changeOrderStatus(@PathVariable("id") Long id,
        ...
        try {
            OrderStatus ns;
            try {
                ns = OrderStatus.valueOf(newStatus);
            } catch (Exception ex) {
                ra.addFlashAttribute("error", "Trạng thái không hợp lệ: " + newStatus);
                return "redirect:/admin/orders";
            }

            // CHẶN cập nhật sang COMPLETED
            if (ns == OrderStatus.COMPLETED) {
                ra.addFlashAttribute("error", "Trạng thái ‘Hoàn tất’ đã bị vô hiệu hoá.");
                return "redirect:/admin/orders";
            }

            orderService.updateStatus(id, ns);
            ra.addFlashAttribute("success", "Đã cập nhật trạng thái đơn #" + id + " → " + ns.getLabel());
        } catch (Exception e) {
    ...
    public ResponseEntity<Map<String, Object>> apiChangeOrderStatus(@PathVariable("id") Long id,
        ...
        try {
            ...
            try {
                ...
                out.put("ok", false);
                out.put("error", "Trạng thái không hợp lệ: " + newStatus);
    ...
    public String saveProduct(
        ...
        try {
            ...
            try {
                com.v.model.Book book = bookService.findById(saved.getId());
                if (book != null) {
                    ...
                    book.setImageUrl("/product/" + saved.getId() + "/image/" + safeFeatured);
                    ...
                    for (int idx = 1; idx <= 5; idx++) {
                        ...
                        if (d != null && d.length > 0) {
                            book.getImages().add(
                                    new com.v.model.BookImage(
                                            book, "/product/" + saved.getId() + "/image/" + idx));
                        }
                    }
